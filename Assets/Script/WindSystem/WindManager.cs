...
public class WindManager : MonoBehaviour
{
    ...
    private List<Rigidbody> windObjects;
    ...
    //{
    //    foreach (var windObject in windObjects)
    //    {
    //        windObject.Wind(direction,strength);
    //    }
    //}
    public bool SubscribeObject(Rigidbody newObject)
    {
        //Esto no deberia dar error nunca, pero por si acaso y de paso mando un bool para que el objeto sepa si ha sido incluido a la lista correctamente
        try
        {

            windObjects.Add( newObject );
            return true;
        }
        catch (Exception e)
        {
    ...
    private bool UnsubscribeObject(Rigidbody newObject)
    {
        try
        {
            windObjects.Remove(newObject);
            return true;
        }
        catch (Exception e)
        {
    ...
    void Rescan()
    {
        ...
        for (int i = 0; i < rbs.Count; i++)
        {
            ...
            if (!rb) continue;
            if (((1 << rb.gameObject.layer) & affectedLayers) == 0) continue;
            if (areaLimited && areaBounds) { if (!areaBounds.bounds.Contains(rb.GetComponent<Rigidbody>().worldCenterOfMass)) continue; }
            targets.Add(rb);
        }
    }
